...
N_vort_sqv = 2  # Number of vortices
pos_sqv = [-5.5, -20, 5.5, -20]
theta_k_sqv = np.empty((N_vort_sqv, Nx, Ny))

N_vort_hqv = 4
pos_hqv = [-5.5, 5.5, -20, 20, 5.5, -5.5, -20, 20]
theta_k_hqv = np.empty((N_vort_hqv, Nx, Ny))
theta_hqv = np.empty((Nx, Ny))
theta_sqv = np.empty((Nx, Ny))

for k in range(N_vort_hqv // 2):
    # Scaling positional arguments
    Y_minus = 2 * pi * (Y - pos_hqv[k]) / len_y
    X_minus = 2 * pi * (X - pos_hqv[N_vort_hqv // 2 + k]) / len_x
    Y_plus = 2 * pi * (Y - pos_hqv[N_vort_hqv + k]) / len_y
    X_plus = 2 * pi * (X - pos_hqv[3 * N_vort_hqv // 2 + k]) / len_x
    x_plus = 2 * pi * pos_hqv[3 * N_vort_hqv // 2 + k] / len_x
    x_minus = 2 * pi * pos_hqv[N_vort_hqv // 2 + k] / len_x
...
for k in range(N_vort_sqv // 2):
    # Scaling positional arguments
    Y_minus = 2 * pi * (Y - pos_sqv[k]) / len_y
    X_minus = 2 * pi * (X - pos_sqv[N_vort_sqv // 2 + k]) / len_x
    Y_plus = 2 * pi * (Y - pos_sqv[N_vort_sqv + k]) / len_y
    X_plus = 2 * pi * (X - pos_sqv[3 * N_vort_sqv // 2 + k]) / len_x
    x_plus = 2 * pi * pos_sqv[3 * N_vort_sqv // 2 + k] / len_x
    x_minus = 2 * pi * pos_sqv[N_vort_sqv // 2 + k] / len_x
...
Psi[0, :, :] = 1 / sqrt(2.) * np.ones((Nx, Ny), dtype='complex128') * exp(1j * theta_hqv)
Psi[1, :, :] = np.zeros((Nx, Ny)) + 0j
Psi[2, :, :] = 1 / sqrt(2.) * np.ones((Nx, Ny), dtype='complex128') * exp(1j * theta_sqv)
...
# ----------------------------------------------------------------------------------------------------------------------
filename = 'mixed_dipole'   # Name of file to save data to
...
for i in range(Nt):
    ...
    # Computing kinetic energy + quadratic Zeeman
    psi_plus_k *= exp(-0.25 * 1j * dt * (Kx ** 2 + Ky ** 2 + 4 * q))
    psi_0_k *= exp(-0.25 * 1j * dt * (Kx ** 2 + Ky ** 2))
    psi_minus_k *= exp(-0.25 * 1j * dt * (Kx ** 2 + Ky ** 2 + 4 * q))
