...
tau_q = 500  # Time when q=q_final (dimensionless units)
c0 = 10
c2 = 0.5
p = c2 * n_0 / 2  # Linear Zeeman

# Framework for wavefunction data
psi_plus = np.sqrt(n_0) * np.sqrt((1 + p / (c2 * n_0)) / 2) + (np.random.normal(0, 1e-4, Nx) +
                                                1j * np.random.normal(0, 1e-4, Nx)) / np.sqrt(Nx)
psi_0 = np.zeros(Nx, dtype='complex64') + (np.random.normal(0, 1e-4, Nx)
                                           + 1j * np.random.normal(0, 1e-4, Nx)) / np.sqrt(Nx)
psi_minus = np.sqrt(n_0) * np.sqrt((1 - p / (c2 * n_0)) / 2) + (np.random.normal(0, 1e-4, Nx)
                                                 + 1j * np.random.normal(0, 1e-4, Nx)) / np.sqrt(Nx)
...
t = -Q_init * tau_q
Q = Q_init
Nt = int(Q_final * tau_q / dt)  # 1.5 is to extend beyond when Q = Q_final
N_steps = Nt // Nframe  # Saves data every N_steps
k = 0  # Array index

filename = '1d_swislocki_{}'.format(tau_q)  # Name of file to save data to
...
with h5py.File(data_path, 'w') as data:
    ...
    data.create_dataset('initial_state/psi_plus', data=(np.fft.ifft(psi_plus_k)))
    data.create_dataset('initial_state/psi_0', data=(np.fft.ifft(psi_0_k)))
    data.create_dataset('initial_state/psi_minus', data=(np.fft.ifft(psi_minus_k)))
