...
n_0 = 1.
psi_plus = cp.sqrt((1 + p / (c2 * n_0)) / 2) + (cp.random.normal(0, 0.5, Nx) +
                                                1j * cp.random.normal(0, 0.5, Nx)) / cp.sqrt(Nx)
psi_0 = cp.zeros(Nx, dtype='complex64') + (cp.random.normal(0, 0.5, Nx)
                                           + 1j * cp.random.normal(0, 0.5, Nx)) / cp.sqrt(Nx)
psi_minus = cp.sqrt((1 - p / (c2 * n_0)) / 2) + (cp.random.normal(0, 0.5, Nx)
                                                 + 1j * cp.random.normal(0, 0.5, Nx)) / cp.sqrt(Nx)
