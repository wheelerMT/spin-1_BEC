...
filename = '1024mag20%'  # Name of file to save data to
...
# If it is a fresh simulation, generate the initial state:
    ...
    new_atom_plus = 1.2 * dx * dy * cp.linalg.norm(cp.fft.ifft2(psi_plus_k)) ** 2
    new_atom_minus = 0.8 * dx * dy * cp.linalg.norm(cp.fft.ifft2(psi_minus_k)) ** 2

    psi_plus_k = cp.fft.fft2(cp.sqrt(new_atom_plus) * cp.fft.ifft2(psi_plus_k)
                             / cp.sqrt(dx * dy * cp.linalg.norm(cp.fft.ifft2(psi_plus_k)) ** 2))
    psi_minus_k = cp.fft.fft2(cp.sqrt(new_atom_minus) * cp.fft.ifft2(psi_minus_k)
                              / cp.sqrt(dx * dy * cp.linalg.norm(cp.fft.ifft2(psi_minus_k)) ** 2))
