...
filename = '1024nomag_newinit'  # Name of file to save data to
...
# If it is a fresh simulation, generate the initial state:
    ...
    np.random.seed(9973)
    theta_k_p = cp.asarray(np.random.uniform(low=0, high=1, size=(Nx, Ny)) * 2 * np.pi)
    np.random.seed(9962)
    theta_k_m = cp.asarray(np.random.uniform(low=0, high=1, size=(Nx, Ny)) * 2 * np.pi)
    ...
    Psi_k[0, :, :] = cp.fft.fftshift(Nx * Ny * cp.sqrt(n_k) * cp.exp(1j * theta_k_p)) / cp.sqrt(2.)
    Psi_k[1, :, :] = cp.zeros((Nx, Ny)) + 0j
    Psi_k[2, :, :] = cp.fft.fftshift(Nx * Ny * cp.sqrt(n_k) * cp.exp(1j * theta_k_m)) / cp.sqrt(2.)
