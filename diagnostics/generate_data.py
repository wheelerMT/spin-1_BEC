...
psi_plus_k = np.empty((Nx, Ny), dtype='complex128')
psi_0_k = np.empty((Nx, Ny), dtype='complex128')
psi_minus_k = np.empty((Nx, Ny), dtype='complex128')
...
N_vort_sqv = 20  # Number of vortices

pos_sqv = [pos_sqv for pos_sqv in np.random.uniform(low=-len_x/2, high=len_x/2, size=2 * N_vort_sqv)]  # Position of dipoles
theta_k_sqv = np.empty((N_vort_sqv, Nx, Ny))
theta_sqv = np.empty((Nx, Ny))

# Generates phase profile with imprinted vortices
for k in range(0, N_vort_sqv, 4):
    # Scaling positional arguments
    Y_minus = 2 * pi * (Y - pos_sqv[k]) / len_y
    X_minus = 2 * pi * (X - pos_sqv[k + 1]) / len_x
    Y_plus = 2 * pi * (Y - pos_sqv[k + 2]) / len_y
    X_plus = 2 * pi * (X - pos_sqv[k + 3]) / len_x
    x_plus = 2 * pi * pos_sqv[k + 3] / len_x
    x_minus = 2 * pi * pos_sqv[k + 1] / len_x

    for nn in np.arange(-5, 5):
        theta_k_sqv[k, :, :] += arctan(
            tanh((Y_minus + 2 * pi * nn) / 2) * tan((X_minus - pi) / 2)) \
                            - arctan(tanh((Y_plus + 2 * pi * nn) / 2) * tan((X_plus - pi) / 2)) \
                            + pi * (heav(X_plus, 1.) - heav(X_minus, 1.))

    theta_k_sqv[k, :, :] -= (2 * pi * Y / len_y) * (x_plus - x_minus) / (2 * pi)
    theta_sqv += theta_k_sqv[k, :, :]

N_vort_hqv = 40
pos_hqv = [pos_sqv for pos_sqv in np.random.uniform(low=-len_x/2, high=len_x/2, size=2 * N_vort_sqv)]
pos_hqv += pos_sqv

theta_k_hqv = np.empty((2 * N_vort_hqv, Nx, Ny))
theta_hqv = np.empty((Nx, Ny))
for k in range(0, N_vort_hqv * 2, 4):
    # Scaling positional arguments
    Y_minus = 2 * pi * (Y - pos_hqv[k]) / len_y
    X_minus = 2 * pi * (X - pos_hqv[k + 1]) / len_x
    Y_plus = 2 * pi * (Y - pos_hqv[k + 2]) / len_y
    X_plus = 2 * pi * (X - pos_hqv[k + 3]) / len_x
    x_plus = 2 * pi * pos_hqv[k + 3] / len_x
    x_minus = 2 * pi * pos_hqv[k + 1] / len_x

    for nn in np.arange(-5, 5):
        theta_k_hqv[k, :, :] += arctan(
            tanh((Y_minus + 2 * pi * nn) / 2) * tan((X_minus - pi) / 2)) \
                            - arctan(tanh((Y_plus + 2 * pi * nn) / 2) * tan((X_plus - pi) / 2)) \
                            + pi * (heav(X_plus, 1.) - heav(X_minus, 1.))

    theta_k_hqv[k, :, :] -= (2 * pi * Y / len_y) * (x_plus - x_minus) / (2 * pi)
    theta_hqv += theta_k_hqv[k, :, :]

# Initial wavefunction
Psi = np.empty((3, Nx, Ny), dtype='complex128')
Psi[0, :, :] = np.ones((Nx, Ny)) * exp(1j * theta_sqv)
Psi[1, :, :] = np.zeros((Nx, Ny)) + 0j
Psi[2, :, :] = np.ones((Nx, Ny)) * exp(1j * theta_hqv)
psi_plus, psi_0, psi_minus = helper.rotation(Psi, Nx, Ny, alpha, beta, gamma)  # Performs rotation to wavefunction

# Artificially generating magnetisation:
new_atom_plus = 1.2 * dx * dy * np.linalg.norm(psi_plus) ** 2
new_atom_minus = 0.8 * dx * dy * np.linalg.norm(psi_minus) ** 2

psi_plus = np.sqrt(new_atom_plus) * psi_plus / np.sqrt(dx * dy * np.linalg.norm(psi_plus) ** 2)
psi_minus = np.sqrt(new_atom_minus) * psi_minus / np.sqrt(dx * dy * np.linalg.norm(psi_minus) ** 2)
...
# ----------------------------------------------------------------------------------------------------------------------
filename = 'vortex'   # Name of file to save data to
data_path = '../data/{}_data.hdf5'.format(filename)
...
for i in range(200):
    ...
    # Forward FFTs
    psi_plus_k = np.fft.fft2(psi_plus)
    psi_0_k = np.fft.fft2(psi_0)
    psi_minus_k = np.fft.fft2(psi_minus)
    ...
    # Inverse FFTs
    psi_plus = np.fft.ifft2(psi_plus_k)
    psi_0 = np.fft.ifft2(psi_0_k)
    psi_minus = np.fft.ifft2(psi_minus_k)
    ...
    if np.mod(i, Nframe) == 0:
        ...
        ax[0].contourf(X, Y, abs(psi_plus) ** 2, levels=25, cmap='gnuplot')
        ax[1].contourf(X, Y, abs(psi_minus) ** 2, levels=25, cmap='gnuplot')
        plt.show()
    t += dt
